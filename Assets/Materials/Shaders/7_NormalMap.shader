		...
		SubShader
		{
			Pass
			{
				...
				float4 frag(vertexOutput i) : COLOR
				{
					...
					if (_WorldSpaceLightPos0.w == 0)  // dirrectional light
					{
						atten = 1.0;
					...
					// localCoords = 1.0 - 0.5 * dot(localCoorss, localCoords); // documentation controlls
					localCoords.z = _BumpDepth; // manual control
					//localCoords.z = 1.0;
			...
			Pass
			{
				...
				float4 frag(vertexOutput i) : COLOR
				{
					...
					// Texture Maps
					//float4 tex = tex2D(_MainTex, i.tex.xy * _MainTex_ST.xy + _MainTex_ST.zw); // deleted for additional lighting
					float4 texN = tex2D(_BumpMap, i.tex.xy * _BumpMap_ST.xy + _BumpMap_ST.zw);
					...
					float3 lightFinal = diffuseReflection + specularReflection + rimLighting; // deleted  UNITY_LIGHTMODEL_AMBIENT.xyz + 

					return float4(lightFinal * _Color.xyz, 1.0); // deleted tex.xyz
				}
			ENDCG
			}
		}
